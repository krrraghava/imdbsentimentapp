...
def preprocess_dataset(df: pd.DataFrame):
    ...
    df["sentiment_label"] = df["sentiment"].str.lower().map(
        {"positive": 1, "negative": 0}
    )
    df = df.dropna(subset=["review", "sentiment_label"])
    with st.spinner("🧹 Cleaning & stemming reviews — this may take a minute…"):
...
# HEADER
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="main-header">🎬 IMDB Sentiment Analyzer</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Upload your IMDB dataset · Choose a vectorizer · Train · Predict</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    use_sample = st.checkbox(
        "Use a subset of rows (faster training)",
        value=True,
    )
...
with tab_train:
    ...
    if "review" not in cols_lower or "sentiment" not in cols_lower:
    ...
    if train_button:
        ...
        X = df_clean["clean_review"].values
    ...
    if st.session_state["model_trained"]:
        ...
        support = int(report_df.loc["Negative", "support"]) + int(report_df.loc["Positive", "support"])
        ...
        with st.expander("ℹ️ Training Configuration Summary"):
            st.json({
                ...
                "random_state":   int(random_state),
                "nb_alpha":       alpha,
                "train_samples":  int(len(y_test) / (test_size / 100) * (1 - test_size / 100)),
...
with tab_predict:
    ...
    if not st.session_state["model_trained"]:
            ...
            if not user_review.strip():
                ...
                with col_res:
                    if prediction == 1:
                        ...
                        st.markdown(
                            '<div style="text-align:center;padding:2rem 0">'
                            '<span class="negative-badge">😞 NEGATIVE</span>'
                            '</div>',
                            unsafe_allow_html=True,
                        )
                ...
                with col_prob:
                    ...
                    fig_prob, ax_prob = plt.subplots(figsize=(4, 1.8))
                    fig_prob.patch.set_facecolor("#1e1e2e")
                    ax_prob.set_facecolor("#1e1e2e")
                    colors = ["#c0392b", "#1a7f37"]
                    bars = ax_prob.barh(
                        prob_df["Sentiment"], prob_df["Probability"],
        ...
        st.markdown(
            ...
            "The app will add a **`predicted_sentiment`** column and let you download it."
        )
        ...
        if batch_file:
            ...
            if "review" not in batch_df.columns:
                ...
                with st.spinner("Running batch predictions…"):
                    batch_df["clean_review"] = batch_df["review"].apply(clean_text)
                    X_batch = st.session_state["vectorizer"].transform(
                        batch_df["clean_review"]
                    )
                ...
                csv_out = batch_df[["review", "predicted_sentiment"]].to_csv(index=False)
                st.download_button(
...
with tab_guide:
    ...
    steps = [
        ("1 · Upload Dataset",
         "Upload a CSV with **review** (text) and **sentiment** (positive/negative) columns. "
         "The IMDB 50K dataset from Kaggle works out of the box."),
        ("2 · Data Preprocessing",
         "HTML tags are stripped, non-alphabetic characters removed, text lowercased, "
         "English stopwords dropped, and each word is reduced to its root via **Porter Stemming**."),
        ("3 · Train / Test Split",
         "Data is split into training and test sets (you control the ratio via the sidebar slider). "
        ...
        ("4 · Vectorization",
         ...
         "**TF-IDF** additionally down-weights common words and up-weights rare but informative ones."),
        ("5 · BernoulliNB Classifier",
         "A Bernoulli Naïve Bayes model is trained on the vectorized data. It assumes binary "
         "presence/absence of words and works excellently for sentiment classification."),
        ("6 · Evaluation",
         "Accuracy, precision, recall, F1-score per class, and the confusion matrix are shown "
         "so you can assess model quality on the unseen test set."),
        ("7 · Predict",
         "Type any review in the **Predict** tab. The app preprocesses it identically, "
         "transforms it with the trained vectorizer, and returns **Positive / Negative** "
    ...
    st.markdown(
        "- Use the full 50 K dataset (uncheck *Use a subset*).  \n"
        "- Try TF-IDF with 20 K–30 K features.  \n"
        "- Keep the test split around 20–30 %.  \n"
